        ...
        button.addEventListener("click", function () {
            ...
            fetch(`/productDetail/review/${reviewId}/like`, {
                ...
                .then(data => {
                    ...
                    // 좋아요 개수 요소 찾기
                    let likeCountElem = document.getElementById(`like-count-${reviewId}`);

                    // 만약 요소가 없으면 새로 생성 (동적 생성)
                    if (!likeCountElem) {
                        console.warn(`🚨 like-count-${reviewId} 요소가 없음. 생성합니다.`);
                        likeCountElem = document.createElement("span");
                        likeCountElem.id = `like-count-${reviewId}`;
                        likeCountElem.classList.add("like-count");
                        button.parentNode.appendChild(likeCountElem); // 버튼 부모 요소에 추가
                    }
